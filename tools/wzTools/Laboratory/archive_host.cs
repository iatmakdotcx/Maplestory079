...
internal class archive_host : laboratory_component_base
{
	...
	private class archive_player : Control
	{
		...
		internal wzaudio audio
		{
			set
			{
				...
				{
					cREATESOUNDEXINFO.fileoffset = (uint)value.offset;
					cREATESOUNDEXINFO.length = (uint)value.size;
					this.system.createSound(value.location, (MODE)34u, ref cREATESOUNDEXINFO, ref this.sound);
				}
	...
	internal override void update_ui_language()
	{
		...
		this.item_version.Text = laboratory_language.query_entry("archive_statusstrip_version") + " : " + this.archives[0].version;
	...
	internal void wzproperty2XmlNode(XmlNode parNode, wzproperty prop)
	{
		foreach (wzproperty current in prop.Values)
		{
			if (current.type == (int)wzproperty.Type.TREE)
			{
				var node = parNode.OwnerDocument.CreateElement("imgdir");
				node.SetAttribute("name", current.identity);
				parNode.AppendChild(node);
				wzproperty2XmlNode(node, current);
			}
			else if (current.type == (int)wzproperty.Type.STRING)
			{
				var node = parNode.OwnerDocument.CreateElement("string");
				parNode.AppendChild(node);
				node.SetAttribute("name", current.identity);
				node.SetAttribute("value", current.data.ToString());
			}
            else if (current.type == (int)wzproperty.Type.INT)
            {
                var node = parNode.OwnerDocument.CreateElement("int");
                parNode.AppendChild(node);
                node.SetAttribute("name", current.identity);
                node.SetAttribute("value", current.data.ToString());
            }
			else if (current.type == (int)wzproperty.Type.SHORT)
            {
                var node = parNode.OwnerDocument.CreateElement("short");
                parNode.AppendChild(node);
                node.SetAttribute("name", current.identity);
                node.SetAttribute("value", current.data.ToString());
            }
			else if (current.type == (int)wzproperty.Type.FLOAT)
            {
                var node = parNode.OwnerDocument.CreateElement("float");
                parNode.AppendChild(node);
                node.SetAttribute("name", current.identity);
                node.SetAttribute("value", current.data.ToString());
            }
            else if (current.type == (int)wzproperty.Type.VECTOR)
            {
                var node = parNode.OwnerDocument.CreateElement("vector");
                parNode.AppendChild(node);
                node.SetAttribute("name", current.identity);
				var data = (wzvector)current.data;
                node.SetAttribute("x", data.x.ToString());
                node.SetAttribute("y", data.y.ToString());
            }
			else if (current.type == (int)wzproperty.Type.CANVAS)
            {
                var node = parNode.OwnerDocument.CreateElement("canvas");
                parNode.AppendChild(node);
                node.SetAttribute("name", current.identity);
				var data = (wzcanvas)current.data;
                node.SetAttribute("width", data.width.ToString());
                node.SetAttribute("height", data.height.ToString());
            }
            else if (current.type == (int)wzproperty.Type.NULL)
			{
				var node = parNode.OwnerDocument.CreateElement("null");
				parNode.AppendChild(node);
				node.SetAttribute("name", current.identity);
			}
			else
			{
				throw new Exception("Î´ÖªÀàÐÍ: " + current.type + "," + prop.identity);
			}
		}
    }
    internal override bool save_package_SvrXml(laboratory_settings settings)
	{
        string savePath = settings.archive_save_location + "\\" + this.Text + "\\";
        Directory.CreateDirectory(savePath);

        var root = this.listview.Tag as wzproperty;
		if (root != null)
		{
			Console.WriteLine(root.identity);
			XmlDocument doc = new XmlDocument();
            doc.AppendChild(doc.CreateXmlDeclaration("1.0", "UTF-8", "yes"));
            var xmlRoot = doc.CreateElement("imgdir");
			doc.AppendChild(xmlRoot);
            xmlRoot.SetAttribute("name", root.identity);
			wzproperty2XmlNode(xmlRoot, root[""]);
			doc.Save(Path.Combine(savePath, root.identity + ".xml"));
            MessageBox.Show("ok");
        }
		else
		{
	...
	internal override bool save_package(bool single, laboratory_settings settings)
	{
		...
		string text = settings.archive_save_location + "\\" + base.invalid_name((this.listview.Tag as wzproperty).identity) + "\\";
	...
	protected override void expand_package()
	{
		...
		wzproperty root = wzpackage.root;
		...
		else if (2 == wzpackage.type || 4 == wzpackage.type)
		{
	...
	private bool save_item(string location, ListViewItem item)
	{
		...
		if (2 == wzproperty.type)
		{
			return (wzproperty.data as wzaudio).save(location + str);
		}
		return 4 != wzproperty.type || (wzproperty.data as wzcanvas).save(location + str);
	}
	...
	private void save_package(object parameter)
	{
		...
		parameter = this.save_package("", (this.listview.Tag as wzproperty)[""], array[0] as string, (bool)array[1], (bool)array[2], (bool)array[3], streamWriter, this.item_progress);
		streamWriter.WriteLine("</table>");
		streamWriter.Close();
		stream.Close();
		base.Invoke(new Action<int>(delegate(int o)
	...
	private string save_package(string prefix, wzproperty proprietor, string location, bool image, bool audio, bool etc, StreamWriter html, ToolStripProgressBar progress)
	{
		...
		foreach (wzproperty current in proprietor.Values)
		{
			...
			string text2 = prefix + "." + current.identity;
			string text3 = text2.Substring(1);
			string str = base.invalid_name(text3);
			if (2 == current.type && audio)
			{
				if (!(current.data as wzaudio).save(location + str))
				{
					text = text + "\r\n" + text3;
				}
			}
			else if (4 == current.type && image && !(current.data as wzcanvas).save(location + str))
			{
				text = text + "\r\n" + text3;
			}
			switch (current.type)
			...
			case 1:
				if (etc)
				{
					this.write_line(text3, (current.data as wzvector).content, html);
				}
				break;
			case 2:
				if (audio)
				{
					this.write_line(text3, (current.data as wzaudio).content, html);
				}
				break;
			case 4:
				if (image)
			...
			case 5:
				if (etc)
				{
					this.write_line(text3, (current.data as wzuol).link, html);
				}
				break;
			default:
				if (etc)
				{
					this.write_line(text3, string.Format("{0}", current.data), html);
				}
				break;
			}
			if (0 < current.Count)
	...
	private void generate_list(wzproperty proprietor)
	{
		...
		this.item_progress.Maximum = this.generate_list("", proprietor[""], list);
	...
	private int generate_list(string prefix, wzproperty proprietor, List<ListViewItem> items)
	{
		...
		foreach (wzproperty current in proprietor.Values)
		{
			int num2 = (2 == current.type) ? 2 : ((4 == current.type) ? 1 : 0);
	...
	protected override void listview_selected_index_changed(object o, EventArgs e)
	{
		...
		if (0 < this.listview.SelectedItems.Count)
		{
			...
			switch (wzproperty.type)
			{
			case 1:
				this.display_etc((wzproperty.data as wzvector).content);
				return;
			case 2:
				this.display_audio(wzproperty.data as wzaudio);
				return;
			case 4:
				this.display_image(wzproperty.data as wzcanvas);
				return;
			case 5:
				this.display_link(wzproperty.data as wzuol);
				return;
			}
			this.display_etc(string.Format("{0}", wzproperty.data));
		}
	}
	...
	private void display_link(wzuol uol)
	{
		if (uol != null)
		{
			if (uol.target == null)
			{
				this.linklabel.Text = uol.link;
			}
			else
			{
				string text = uol.target.absolute.Replace('/', '.').Substring(1);
