...
internal class animation_host : laboratory_component_base
{
	private class animation_raws : Dictionary<string, animation_host.animation_raw>
	{
		...
		internal animation_raws(wzproperty property)
		{
			...
			this.enumerate_animations(property[""]);
		}
		...
		{
			animation_host.animation_raw animation_raw = new animation_host.animation_raw(proprietor.identity);
			foreach (wzproperty current in proprietor.Values)
			{
				int num;
				if (int.TryParse(current.identity, out num))
				{
			...
			{
				base.Add(proprietor.absolute.Replace('/', '.').Substring(1), animation_raw);
			}
		}
		...
		private void valid_frame(wzproperty property, animation_host.animation_raw animation)
		{
			...
			if (5 == property.type)
			{
				this.valid_frame((property.data as wzuol).target, animation);
			}
		}
	}
	...
	private class animation_raw : List<wzproperty>
	{
		...
		internal animation_host.animation animation
		{
			get
			{
				...
				{
					wzcanvas canvas = current.data as wzcanvas;
					wzproperty wzproperty;
					wzproperty wzproperty2;
					list.Add(new animation_host.animation_property(canvas, current.TryGetValue("origin", out wzproperty) ? (wzproperty.data as wzvector) : new wzvector(0, 0), current.TryGetValue("delay", out wzproperty2) ? int.Parse(string.Format("{0}", wzproperty2.data)) : 100));
				}
				int x = list[0].x;
				int y = list[0].y;
				int num = -x;
		...
		{
			Image image = property.canvas.image;
	...
	private class animation_property
	{
		...
		internal int width
		{
			get
			{
				return (this.properties[0] as wzcanvas).width;
			}
		}
		...
		internal int height
		{
			get
			{
				return (this.properties[0] as wzcanvas).height;
			}
		}
