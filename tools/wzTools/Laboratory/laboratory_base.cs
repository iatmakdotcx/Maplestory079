...
internal abstract class laboratory_base : Form
{
	...
	{
		var laboratory_font_family = laboratory_language.query_entry("laboratory_font_family");
		Font font;
        if (!string.IsNullOrEmpty(laboratory_font_family) && float.TryParse(laboratory_language.query_entry("laboratory_font_size"),out var laboratory_font_size) && laboratory_font_size>0)
		{
			font = new Font(laboratory_font_family, laboratory_font_size);
		}
		else
		{
			font = Font;
