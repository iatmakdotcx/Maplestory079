...
public class AdminCommand {
    ...
    public static class GainCash extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!gaingash <数量> <玩家> - 取得Gash点数";
        }
    }
    ...
    public static class GainMaplePoint extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!gainmaplepoint <数量> <玩家> - 取得枫叶点数";
        }
    }
    ...
    public static class GainPoint extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!gainpoint <数量> <玩家> - 取得Point";
        }
    }
    ...
    public static class 给所有人道具 extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            if (splitted.length > 2) {
                ...
                c.getPlayer().dropMessage(6, "命令使用成功，当前共有: " + ret + " 个玩家获得: " +
                        quantity + " 个" + ii.getName(itemId));
            } else {
...
//                monster.setFh(c.getPlayer().getMap().getFootholds().findBelow(c.getPlayer().getPosition()).getId());
                c.getPlayer().getMap().broadcastMessage(MobPacket.moveMonster(true, 0, 0, monster.getObjectId(),
                        c.getPlayer().getPosition(), c.getPlayer().getPosition(), c.getPlayer().getLastRes()));
            }
            return true;
        }
    ...
    public static class ItemVac extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(6, "目前自动捡物状态:" + (!ItemVac ? "开启" : "关闭"));
            return true;
    ...
    public static class LockItem extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!lockitem <角色名称> <物品ID> - 上锁玩家身上的道具";
        }
    }
    ...
    public static class SendAllNote extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!sendallnote <文字> 传送Note给目前频道的所有人";
        }
    }
    ...
    public static class MesoEveryone extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!mesoeveryone <数量> - 给所有玩家金币";
        }
    }
    ...
    public static class ToggleMegaphone extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!togglemegaphone - 开启或关闭广播";
    ...
    public static class HitMonsterByOID extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!hitmonsterbyoid <moboid> <damage> - 碰撞地图上某个怪物";
    ...
    public static class NPC extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            try {
                ...
                c.getPlayer().dropMessage(6, getMessage());
            }
    ...
    public static class MakePNPC extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!makepnpc <playername> <npcid> - 创造玩家NPC";
        }
    }
    ...
    public static class MakeOfflineP extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!deletepnpc <charname> <npcid> - 创造离线PNPC";
        }
    }
    ...
    public static class DestroyPNPC extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!destroypnpc [objectid] - 删除PNPC";
        }
    ...
    public static class StartProfiling extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!startprofiling 开始纪录JVM资讯";
        }
    }
    ...
    public static class StopProfiling extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!stopprofiling <filename> - 取消纪录JVM资讯并储存到档案";
        }
    }
    ...
    public static class Packet extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            c.getPlayer().dropMessage(packetheader + "已传送封包[" + packetheader + "] : " + mplew);
            return true;
        }
    ...
    public static class ProDrop extends CommandExecute {
        ...
        @Override
        public String getMessage() {
            return "!ProDrop <物品代码> (<力量> <敏捷> <智力> <幸运> <HP> <MP> <物攻> <魔攻> <物防> <魔防> <武器+x> <命中> <回避> <移动> <跳跃> <冲卷数> <天数>)";
        }
    }
    ...
    public static class 给点数 extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            ...
            try {
                ...
                error = "请输入数字以及不能给予超过2147483647的 " + input + " 错误为: " + ex;
            }
            if (error != null) {
    ...
    public static class CashEveryone extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            if (splitted.length > 2) {
                ...
                c.getPlayer().dropMessage(6,
                        "命令使用成功，当前共有: " + ret + " 个玩家获得: " + quantity +
                                " 点的" + (type == 1 ? "GASH " : " 枫叶点数 ") + " 共计: " +
                                ret * quantity);
            } else {
                c.getPlayer().dropMessage(6, "用法: !给所有人点数 [点数类型1-2] [点数数量] 1是GASH.2是枫叶点数");
            }
    ...
    public static class 附近NPC extends CommandExecute {
        ...
        public boolean execute(MapleClient c, String[] splitted) {
            List npcs;
            if(splitted.length>=2 && "all".equals(splitted[1]))
            {
                npcs = c.getPlayer().getMap().getAllNPCsThreadsafe();
            }else {
                npcs = c.getPlayer().getMap().getMapObjectsInRange(c.getPlayer().getPosition(), 10000, Collections.singletonList(MapleMapObjectType.NPC));
            }
            for (final Object obj :npcs) {
                MapleNPC npc = (MapleNPC) obj;
                c.getPlayer().dropMessage(6, "NPC " + npc);
            }
            if (npcs.isEmpty()) {
                c.getPlayer().dropMessage(6, "附近没有Npc");
            }
            return true;
        }
        ...
        @Override
        public String getMessage() {
            return "!附近NPC - 查看角色附近NPC";
        }
    }
    public static class setNpc extends CommandExecute {

        @Override
        public boolean execute(MapleClient c, String[] splitted) {
            MapleNPC npc = c.getPlayer().getMap().getNPCById(Integer.parseInt(splitted[1]));
            if(npc==null) {
                c.getPlayer().dropMessage(6, "当前地图没有此NPC。" + splitted[1]);
                return true;
            }
            ...
            switch (splitted[2])
            {
                ...
                case "position":
                    ...
                    break;
                default:
                    c.getPlayer().dropMessage(6, "无效类型：" + splitted[2]);
                    break;
            }
            try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {


                try (PreparedStatement ps =  con.prepareStatement("delete from wz_customlife where mid=? and dataid=?")) {
                ...
                try (PreparedStatement ps = con.prepareStatement("INSERT INTO wz_customlife (dataid, f, hide, fh, cy, rx0, rx1, type, x, y, mid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
                    ...
                    ps.setInt(2, npc.getF()); // 1 = right , 0 = left
                    ps.setInt(3, npc.isHidden()?1:0); // 1 = hide, 0 = show
                    ps.setInt(4, npc.getFh());
                    ps.setInt(5, npc.getCy());
                    ps.setInt(6, npc.getRx0());
                    ps.setInt(7, npc.getRx1());
                    ps.setString(8, "n");
                    ps.setInt(9, npc.getPosition().x);
                    ps.setInt(10, npc.getPosition().y);
                    ps.setInt(11, c.getPlayer().getMapId());
                    ps.executeUpdate();
                }
            } catch (SQLException e) {
            ...
            c.getPlayer().getMap().refreshNpc(npc);
            return true;
        }

        @Override
        public String getMessage() {
            return "!setNpc <ID> [hide/show/position] - 设置npc信息";
        }
    }
}
