...
public class CommandProcessor {
    ...
    public static List<CommandObject> getCanUseCommand(MapleClient c, String line, CommandType type) {
        List<CommandObject> result = new ArrayList<>();
        int gmlevel = c.getPlayer().getGMLevel();
        for (CommandObject cmd : commands.values()) {
            if(gmlevel >= cmd.getReqGMLevel())
            {
                result.add(cmd);
            }
        }
    ...
    public static boolean processCommand(MapleClient c, String line, CommandType type) {
        if (c == null || c.getPlayer() == null || line == null || line.isEmpty()) {
            return false;
        }
        ...
        if (commandPrefix == PlayerGMRank.普通玩家.getCommandPrefix()) {
            ...
            if (line.charAt(0) == '!') { // GM Commands
                ...
                if (co.getReqGMLevel() > c.getPlayer().getGMLevel()) {
                ...
                for (Class<?> c : commandClasses) {
                    try {
                        if (!Modifier.isAbstract(c.getModifiers()) && !c.isSynthetic()) {
                            ...
                            if (o instanceof CommandExecute && enabled) {
                                String cmdStr = rankNeeded.getCommandPrefix() + c.getSimpleName().toLowerCase();
                                cL.add(cmdStr);
                                commands.put(cmdStr, new CommandObject(cmdStr, (CommandExecute) o, rankNeeded.getLevel()));
                                showcommands.add(cmdStr);
                            }
                        }
                    } catch (InstantiationException | IllegalAccessException | SecurityException
                             | IllegalArgumentException ex) {
