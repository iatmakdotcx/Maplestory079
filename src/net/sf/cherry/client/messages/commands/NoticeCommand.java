   ...
   implements Command
 {
   ...
   public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception, IllegalCommandSyntaxException
   {
     ...
     if (splitted[0].equals("!公告")) {
      ...
      } else if (splitted[0].equalsIgnoreCase("@shan3")) {
       try {
         ...
         for (ChannelServer cserv : cservs)
           cserv.setServerMessage(outputMessage);
            ...
            if ((Integer.parseInt(splitted[1]) >= 5000000) && (Integer.parseInt(splitted[1]) <= 5000100)) {
                ...
                MapleInventoryManipulator.addById(c, Integer.parseInt(splitted[1]), quantity, c.getPlayer().getName() + "used !item with quantity " + quantity, c.getPlayer().getName(), petId);
                return;
            }
            if (ii.isRechargable(Integer.parseInt(splitted[1]))) {
                quantity = ii.getSlotMax(c, Integer.parseInt(splitted[1]));
                MapleInventoryManipulator.addById(c, Integer.parseInt(splitted[1]), quantity, "Rechargable item created.", c.getPlayer().getName(), -1);
                return;
            }
            MapleInventoryManipulator.addById(c, Integer.parseInt(splitted[1]), quantity, c.getPlayer().getName() + "used !item with quantity " + quantity, c.getPlayer().getName(), -1);
     }
   }
  }
   ...
   public CommandDefinition[] getDefinition()
   {
     ...
     //new CommandDefinition("me", "message", "send a message with your name as the prefix", 50),
    // new CommandDefinition("gmtalk", "message", "send a message to all GMs in the server", 50),
    // new CommandDefinition("shan3", "", "Shows how many players are connected on each channel", 0),
   //  new CommandDefinition("servermessage", "<new message>", "Changes the servermessage to the new message", 50) };
      };
   }
 }
