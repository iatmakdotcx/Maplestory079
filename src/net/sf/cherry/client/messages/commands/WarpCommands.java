...
public class WarpCommands implements Command {
    ...
    public void execute(MapleClient c, MessageCallback mc, String[] splitted) throws Exception, IllegalCommandSyntaxException {
        ...
        if (splitted[0].equals("!id")) {
        ...
        } else if (splitted[0].equals("!拉玩家过来")) {
            MapleCharacter victim = cserv.getPlayerStorage().getCharacterByName(splitted[1]);
            ...
            }	else if (splitted[0].equals("!拉所有玩家到身边")) {
                	for (MapleCharacter mch : cserv.getPlayerStorage().getAllCharacters())
                    	if (mch.getMapId() != c.getPlayer().getMapId())
                        mch.changeMap(c.getPlayer().getMap(), c.getPlayer().getPosition());
        } else if (splitted[0].equals("!拉所有玩家到")) {
...
//!warpalive 280030000 
    ...
    public CommandDefinition[] getDefinition() {
        return new CommandDefinition[]{
                    new CommandDefinition("id", "playername", "Warps yourself to the player", 3),
                    new CommandDefinition("id1", "playerid", "Warps yourself to the player", 3),
                    new CommandDefinition("拉所有玩家到身边", "playerid", "Warps yourself to the player", 1),
                    new CommandDefinition("拉所有玩家到", "playername", "Warps the player with the given name to yourself", 3),
                    new CommandDefinition("拉玩家过来", "playername", "Warps the player with the given name to yourself", 4),
                    //new CommandDefinition("offlinewarp", "playername mapid", "Warps a player whilst they are offline", 50),
                    new CommandDefinition("map", "mapid", "Warps you to the given mapid (use /m instead)", 50)
                   // new CommandDefinition("warpall", "mapid", "Warps you and everyone in your map to the mapid specified", 5),
