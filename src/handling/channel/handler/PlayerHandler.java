...
public class PlayerHandler {
    ...
    public static final void UseChair(final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        // return;
        // }
        if (itemId == 3011000 && GameConstants.isFishingMap(chr.getMapId())) {
            boolean haz = false;
            for (IItem item : c.getPlayer().getInventory(MapleInventoryType.CASH).list()) {
                if (item.getItemId() == 5340000) { //钓竿
                    haz = true;
                } else if (item.getItemId() == 5340001) { //5340001
                    haz = false;
...
//            }
            ...
            if ((combo > 0) && (curr - chr.getLastCombo() > 2500L)) {
                combo = 0;
            }
            ...
            switch (combo) {
                ...
                case 90:
                case 100:
                    if (playerSkillLevel < combo / 10) {
                        break;
                    }
                    sk.getEffect(combo / 10).applyComboBuff(chr, combo);
            }
        }
    }
