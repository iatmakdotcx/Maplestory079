...
public class InventoryHandler {
    ...
    public static final boolean UseRewardItem(final byte slot, final int itemId, final MapleClient c,
         ...
         * if (reward.getProb() > 0 && Randomizer.nextInt(rewards.getLeft()) <
         * reward.getProb()) { // Total prob
         ...
         * item.setExpiration(System.currentTimeMillis() + (reward.getPeriod() * 60 * 60
         * * 10));
         * }
         * MapleInventoryManipulator.addbyItem(c, item);
         * } else {
         * MapleInventoryManipulator.addById(c, reward.getItemid(),
         * reward.getQuantity());
         * }
         ...
         * MaplePacketCreator.showRewardItemAnimation(reward.getItemid(),
         * reward.getEffect(), chr.getId()), false);
         * rewarded = true;
         * return rewarded;
         * }
         * }
         ...
         * MapleInventoryManipulator.removeById(c,
         * GameConstants.getInventoryType(itemId), itemId, 1, false, false);
         * c.sendPacket(MaplePacketCreator.getShowItemGain(reward, (short) amount,
         * true));
         * if (rareness > 0) {
         * World.Broadcast.broadcastMessage(MaplePacketCreator.
         * getGachaponMega("	恭喜 " + c.getPlayer().getName(), " : 从" + box + "抽到！",
         * item, rareness));
         * }
         * } else {
    ...
    public static final void UseMagnify(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (eqq.getState() == 1 && (magnify.getItemId() == 2460003 || (magnify.getItemId() == 2460002 && reqLevel <= 12)
            ...
            c.sendPacket(
                    MaplePacketCreator.modifyInventory(true, new ModifyInventory(ModifyInventory.Types.MOVE, magnify)));
            // c.sendPacket(MaplePacketCreator.scrolledItem(magnify, toReveal, false,
            // true));
            c.getPlayer().getMap()
                    .broadcastMessage(MaplePacketCreator.getPotentialReset(c.getPlayer().getId(), eqq.getPosition()));
    ...
    public static final boolean UseUpgradeScroll(final byte slot, final byte dst, final byte ws, final MapleClient c,
        ...
        chr.getMap().broadcastMessage(chr,
                MaplePacketCreator.getScrollEffect(c.getPlayer().getId(), scrollSuccess, legendarySpirit), vegas == 0);
        ...
        // c.sendPacket(MaplePacketCreator.modifyInventory(false, new
        // ModifyInventory(ModifyInventory.Types.UPDATE, scroll)));
        return true;
    }
    ...
    public static final void UseScriptedNPCItem(final LittleEndianAccessor slea, final MapleClient c,
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId) {
            switch (toUse.getItemId()) {
                 ...
                 * MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, 2430112, 25,
                 * true, false)) {
                 ...
                 * }
                 * } else if
                 * (c.getPlayer().getInventory(MapleInventoryType.USE).countById(2430112) >= 10)
                 * {
                 * if (MapleInventoryManipulator.checkSpace(c, 2049400, 1, "") &&
                 * MapleInventoryManipulator.removeById(c, MapleInventoryType.USE, 2430112, 10,
                 * true, false)) {
    ...
    public static final void UseTreasureChest(final LittleEndianAccessor slea, final MapleClient c,
        ...
        if (chr.getInventory(MapleInventoryType.CASH).countById(keyIDforRemoval) > 0) {
            if (chr.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() > -1
                ...
                if (rareness > 0) {
                    World.Broadcast.broadcastMessage(MaplePacketCreator
                            .getGachaponMega("	恭喜 " + c.getPlayer().getName(), " : 从" + box + "抽到！", item, rareness));
                }
            } else {
                chr.dropMessage(5, "你有一个栏位满了，请空出来再打开" + box + "！");
                c.sendPacket(MaplePacketCreator.enableActions());
    ...
    public static final void UseCashItem(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5561000: { // 任意门高级券
                if (slea.readByte() == 0) {
                    ...
                    if (itemId == 5560000 || itemId == 5561000
                            || (itemId != 5560000 || itemId != 5561000 && c.getPlayer().isRegRockMap(target.getId()))) {
                ...
                if (used) {
                    switch (apto) { // AP to
                        ...
                        case MAXHP: // hp
                             ...
                             * } else if ((job >= 300 && job <= 322) || (job >= 400 && job <= 434) || (job
                             * >= 1300 && job <= 1312) || (job >= 1400 && job <= 1412) || (job >= 3300 &&
                             * job <= 3312)) { // Bowman
                            ...
                            if (GameConstants.is新手职业(job)) { // Beginner
                                ...
                                maxhp += 6;
                            } else if ((job >= 300 && job <= 332) || (job >= 400 && job <= 434)
                        ...
                        case MAXMP: // mp
                             ...
                             * }
                             * } else if ((job >= 300 && job <= 322) || (job >= 400 && job <= 434) || (job
                             * >= 500 && job <= 522) || (job >= 3200 && job <= 3212) || (job >= 3500 && job
                             * <= 3512) || (job >= 1300 && job <= 1312) || (job >= 1400 && job <= 1412) ||
                    ...
                    switch (apfrom) { // AP from
                        ...
                        case MAXHP: // HP
                             ...
                             * } else if ((job >= 300 && job <= 322) || (job >= 400 && job <= 434) || (job
                             * >= 1300 && job <= 1312) || (job >= 1400 && job <= 1412) || (job >= 3300 &&
                             * job <= 3312) || (job >= 3500 && job <= 3512)) { // Bowman, Thief
                        ...
                        case MAXMP: // MP
                             ...
                             * }
                             * } else if ((job >= 500 && job <= 522) || (job >= 300 && job <= 322) || (job
                             * >= 400 && job <= 434) || (job >= 1300 && job <= 1312) || (job >= 1400 && job
                             * <= 1412) || (job >= 1500 && job <= 1512) || (job >= 3300 && job <= 3312) ||
                             * (job >= 3500 && job <= 3512)) { // Pirate, Bowman. Thief
            ...
            case 5570000: { // Vicious Hammer
                ...
                if (item != null) {
                    if (GameConstants.canHammer(item.getItemId())
                        ...
                        // c.sendPacket(MTSCSPacket.ViciousHammer(true, (byte)
            ...
            case 5070000: { // Megaphone
                ...
                if ((!c.getPlayer().getCanTalk() || c.getChannelServer().getMegaphoneMuteState())
                        && !c.getPlayer().isGM()) {
                ...
                if (ServerConfig.LOG_MEGA) {
                    FileoutputUtil.logToFile("logs/聊天/广播频道.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: "
                            + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家『" + c.getPlayer().getName()
                            + "』频道『" + c.getChannel() + "』广播道具『" + itemId + "』：" + message);
                }
                used = true;
            ...
            case 5077000: { // 3 line Megaphone
                ...
                for (int i = 0; i < numLines; i++) {
                    ...
                    /*
                     * if (c.getPlayer().isPlayer() &&
                     * !PiPiConfig.isCanTalkText(messages.toString())) {
                     * c.getPlayer().dropMessage("说脏话是不礼貌的，请勿说脏话。");
                     * c.sendPacket(MaplePacketCreator.enableActions());
                     * return;
                     * }
                ...
                if (ServerConfig.LOG_MEGA) {
                    FileoutputUtil.logToFile("logs/聊天/广播频道.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: "
                            + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家『" + c.getPlayer().getName()
                            + "』频道『" + c.getChannel() + "』广播道具『" + itemId + "』：" + messages);
                }
                used = true;
                break;
            }
            case 5072000: { // 高效能喇叭
                ...
                if (ServerConfig.LOG_MEGA) {
                    FileoutputUtil.logToFile("logs/聊天/广播频道.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: "
                            + c.getSession().remoteAddress().toString().split(":")[0] + " " + FileoutputUtil.NowTime()
                            + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家『"
                            + c.getPlayer().getName() + "』频道『" + c.getChannel() + "』广播道具『" + itemId + "』：" + message);
                }
                used = true;
                break;
            }
            ...
            case 5075005: {
                ...
                World.Broadcast.broadcastSmega(MaplePacketCreator.serverNotice(3, c.getChannel(),
            ...
            case 5170000: {
                ...
                pet.setName(nName);
                c.sendPacket(PetPacket.updatePet(pet, c.getPlayer().getInventory(MapleInventoryType.CASH)
            ...
            case 5240028: { // Pet food
                ...
                c.sendPacket(PetPacket.updatePet(pet,
            ...
            case 5281000: {
                ...
                c.getPlayer().getMap()
                        .broadcastMessage(MaplePacketCreator.getChatText(c.getPlayer().getId(), "哦，不，我放屁!", false, 1));
                c.sendPacket(MaplePacketCreator.enableActions());
                used = true;
                break;
            }
            case 5320000: {
                ...
                // pcf.getPredictCardComment(commentId);
                ...
                c.getPlayer().getMap().broadcastMessage(MaplePacketCreator
                        .startMapEffect(name + "和" + otherName + "的速配程度 " + Comment.worldmsg0, 5120026, true));
                used = true;
                break;
            }
            /* 黑板 */
            ...
            case 5390006: { // Tiger Roar's Messenger
                ...
                if (ServerConfig.LOG_MEGA) {
                    FileoutputUtil.logToFile("logs/聊天/广播频道.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: "
                            + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家『" + c.getPlayer().getName()
                            + "』频道『" + c.getChannel() + "』广播道具『" + itemId + "』：" + text);
                }
                used = true;
            ...
            case 5450000: { // Mu Mu the Travelling Merchant
                ...
                } else if (c.getPlayer().getMap().getSquadByMap() != null || c.getPlayer().getEventInstance() != null
                        || c.getPlayer().getMap().getEMByMap() != null || c.getPlayer().getMapId() >= 990000000) {
                    c.getPlayer().dropMessage(5, "你不能在这张地图里使用，如果卡住请使用 @ea 来解卡。");
            ...
            default:
                if (itemId / 10000 == 512) {
                    ...
                    MapleKite Kite = new MapleKite(c.getPlayer(), c.getPlayer().getPosition(),
         ...
         * c.getPlayer().changeChannel(c.getChannel() == ChannelServer.getChannelCount()
         * ? 1 : (c.getChannel() + 1));
         * }
         */
        if (cc) {
        ...
        try {
            ...
            if (Distance > 2500) {
                ...
                chr.getCheatTracker().registerOffense(CheatingOffense.ITEMVAC_SERVER,
                        " 范围: " + chr.getPosition().distanceSq(mapitem.getPosition()));
            }
    ...
    public static final void PetPickup(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            if (count > range) {// Pet loot Hack
                ...
                // 地图: " + chr.getMapId() + " 捡物品范围: " + (long) count);
            }
    ...
    private static boolean getIncubatedItems(MapleClient c) {
        ...
        return MapleInventoryManipulator.addById(c, ids[z], (short) 1)
    ...
    public static final void UseSkillBook(final LittleEndianAccessor slea, final MapleClient c,
        ...
        for (; ; ) {
            ...
            if (Math.floor(CurrentLoopedSkillId / 10000) == chr.getJob()) {
                ...
                if (chr.getSkillLevel(CurrSkillData) >= ReqSkillLevel
        ...
        c.getPlayer().dropMessage(5, "你还没有学会此技能。" );
    ...
    public static void UsePenguinBoxSunzi(final LittleEndianAccessor slea, MapleClient c) {
    ...
    public static void UsePenguinBox(final LittleEndianAccessor slea, MapleClient c) {
        ...
        // if (gift.isEmpty() && item != 2022428 && item != 2022465 && item != 2022466
        // && item != 2022467 && item != 2022468 && item != 2022336 && item != 2022615
        // && item != 2022503 && item != 2022514 && item != 2022428 && item != 2022504
        // && item != 2022505 && item != 2022506 && item != 2022507 && item != 2022508
        // && item != 2022509 && item != 2022510 && item != 2022511 && item != 2022512
        // && item != 2022513 && item != 2022514 && item != 2022515 && item != 2022516
        // && item != 2022517 && item != 2029999) {//2022504
        // } else {
    ...
    // 孙子兵法
    /*
     * public static void SunziBF(final LittleEndianAccessor slea, final MapleClient
     * c) {
     * slea.readInt();
     * byte slot = (byte) slea.readShort();
     * int itemid = slea.readInt();
