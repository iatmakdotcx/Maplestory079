...
public class LoginPacket {
    ...
    public static final byte[] getServerList(WorldConstants.Option world, MapleClient c) {
        final MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
        ...
        if(c.getGender()==1)
        {
            mplew.writeMapleAsciiString("yo! 美女");
        }else{
            mplew.writeMapleAsciiString("yo! 帅哥");
        }
    ...
    public static final byte[] getCharList(final boolean secondpw, final List<MapleCharacter> chars, int charslots) {
        ...
        for (final MapleCharacter chr : chars) {
            addCharEntry(mplew, chr, !chr.getClient().isGm() && chr.getLevel() >= 10);
        }
