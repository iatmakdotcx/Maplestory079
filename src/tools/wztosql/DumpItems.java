...
public class DumpItems {
    ...
    public static void main(String[] args) {
        final MapleDataProvider string = MapleDataProviderFactory.getDataProvider(new File((System.getProperty("wzpath") != null ? System.getProperty("wzpath") : "") + "wz/String.wz"));
        final MapleData cashStringData = string.getData("Cash.img");
        try {
            Connection con = DBConPool.getInstance().getDataSource().getConnection();
            for (MapleData file : cashStringData.getChildren()) {
                //System.out.println(file);
                int id = Integer.parseInt(file.getName());
                PreparedStatement ps = null;
                try {
                    ps = con.prepareStatement("update wz_itemdata set cash=1 where itemid=? ");
                    ps.setInt(1, id);
                    ps.execute();
    ...
    public static void main_eqp(String[] args) {
        final MapleDataProvider string = MapleDataProviderFactory.getDataProvider(new File((System.getProperty("wzpath") != null ? System.getProperty("wzpath") : "") + "wz/String.wz"));
        final MapleData cashStringData = string.getData("Eqp.img");
        try {
            Connection con = DBConPool.getInstance().getDataSource().getConnection();
            for (MapleData file : cashStringData.getChildren()) {
                //System.out.println(file);
                String name = file.getName();
                for (MapleData file1 : file.getChildren()) {
                    String name1 = file1.getName();
                    for (MapleData file2 : file1.getChildren()) {
                        int id = Integer.parseInt(file2.getName());
                        PreparedStatement ps = null;
                        try {
                            ps = con.prepareStatement("update wz_itemdata set type=? where itemid=? ");
                            ps.setString(1, (name+"."+name1));
                            ps.setInt(2, id);
                            ps.execute();
                        } catch (SQLException e) {
                            e.printStackTrace();
