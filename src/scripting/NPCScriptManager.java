...
public class NPCScriptManager extends AbstractScriptManager {
    ...
    public final void start(final MapleClient c, int npc, final int mode, String script) {
        if(c == null || c.getPlayer() == null) {
            System.err.println("NonPlayer script triggered ["+script+"].");
            return;
        }
        if (c.getPlayer().getDebugMessage()) {
            System.out.println("[NPC 交互] npc："+npc+","+mode+","+script);
        }
        ...
        try {
            ...
            if (!cms.containsKey(c) && c.canClickNPC()) {
                ...
                if (iv == null) {
                    ...
                    dropMessage(c, "[scripts提示] 您已经建立与notcoded的对话！npc：" +npc+","+mode+","+script);
                    if (iv == null) {
                        dispose(c);
                        return;
                    }
