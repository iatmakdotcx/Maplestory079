...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNextPrev("呃……嗯？是谁？这声音从哪里传来的？", 2);
    } else if (status == 2) {
        qm.sendNextPrev("你别惊讶，好好听我说。我是你的前任，先代狂龙战士。");
    } else if (status == 3) {
        qm.sendNextPrev("先……先代狂龙战士？", 2);
    } else if (status == 4) {
        qm.sendAcceptDeclineNoESC("你做好准备听我说了吗？");
    } else if (status == 5) {
	if (qm.getJob() == 6111) {
	    qm.changeJob(6112);
	}
	...
	qm.completeQuest();
        ...
        qm.sendNextPrev("我听说冒险岛世界里有生物拥有精灵之力。那生物的名字好像叫“古瓦洛”。总而言之，估计麦格纳斯是获得了那股力量，才得以延续生命。", 2);
    } else if (status == 12) {
        qm.sendNextPrev("过去的事情就说到这儿。事实上我跟你说话是因为你已经做好准备接受我的精髓。");
    } else if (status == 13) {
        qm.sendNextPrev("嗯？精髓？", 2);
    } else if (status == 14) {
