...
function start(mode, type, selection) {
	...
	qm.sendNext("Every hero has to take their first slep, #h #, Your first slep will be into Ereve, All preparations are complete, you only need to exit the ship and descent down.", 1);
    } else if (status == 2) {
	qm.sendPlayerToNpc("This is no problem for the world's greatest thief!");
    } else if (status == 3) {
	qm.sendNextPrev("You may leave now, Your journey begins here.", 1);
    } else if (status == 4) {
	qm.forceStartQuest();
	qm.dispose();
    }
