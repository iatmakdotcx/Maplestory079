...
function end(mode, type, selection) {
	...
	qm.sendNext("#b(You show Mir to Dad.)#k", 2);
    } else if (status == 2) {
	...
	qm.sendNext("#b(Dad doesn't seem to belive that Mir is a Dragon. Well, he is pretty small. Would Dad believe it if he heard Mir talk?)", 2);
    } else if (status == 4) {
	...
	qm.sendNext("#b(What?! Kill Mir?! But he didn't do anything wrong!!)", 2);
    } else if (status == 6) {
	qm.sendNextPrev("Of course, I'm pretty sure it's not a Dragon. Dragons only appear in Leafre on the Ossyria Continent.");
    } else if (status == 7) {
	qm.sendNext("#bHa... Ha... You're definitely right! I doubt he's a Dragon. He's probably just a lizard! Definitely!#k", 2);
    } else if (status == 8) {
	qm.sendNextPrev("Yeah, I'm pretty sure. It's a bizarre-looking lizard, but it doesn't look dangerous. Guess you can keep it.");
    } else if (status == 9) {
	qm.sendNext("#b(For his own safety, you better not let anyone know that Mir is a Dragon.)#k", 2);
    } else if (status == 10) {
