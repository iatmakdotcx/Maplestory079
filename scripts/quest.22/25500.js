...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNextPrev("#b所以简单概括起来，就是使用光之魔法时，消耗黑暗点数就能施展暗之魔法；使用暗之魔法时，消耗光明点数就能改为光之魔法，对吧？", 2);
    } else if (status == 17) {
        qm.sendNextPrev("没错，就是这样。如果你不知道现在该使用什么魔法，可以看看点数栏下方显示的强化的技能。\r\n<光之魔法>              <暗之魔法>\r\n#v3800312#  #v3800315#\r\n你都听明白了吗？");
    } else if (status == 18) {
        qm.sendNextPrev("#b嗯，估计得试一试才能完全理解。", 2);
    } else if (status == 19) {
        qm.sendAcceptDecline("哈哈哈，实践出真知，亲自使用一下你会更明白的。那么你去打猎怪物，#b分别转变一次太阳火焰或月蚀状态#k怎么样？");
    } else if (status == 20) {
        qm.forceStartQuest();
