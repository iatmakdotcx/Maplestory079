...
if (mode == -1) {
	...
	if (status == 0) {
	    ...
	    qm.sendNextPrev("#bI'd like to look through the childrens' rooms.",6);	
    } else if (status == 2) {	 
	    qm.sendNext("Do you think you can find some clues in their things? That might actually work...");
    } else if (status == 3) {
	    qm.sendNextPrev("You're just looking for a way to steal from us! Headmistress Ivana, we must not listen to these cunning strangers!",4,1500002);	
    } else if (status == 4) {
	    ...
	    qm.sendNextPrev("I've got my eye on you, outsider.",4,1500002);	
 	} else if (status == 7) {
	    qm.sendNextPrev("We'll find those precious children, no matter what it takes! I'll meet you on the second floor.\r\n#b(Head to the 2nd floor of Ellinel Fairy Academy and meet Cootie the Really Small.)",4,1500000);	
	} else if (status == 8) {
		qm.completeQuest();
		qm.gainExp(1600);
	    qm.dispose();		
