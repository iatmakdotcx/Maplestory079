...
function start(mode, type, selection) {
	...
	qm.sendNextPrev("#bI see. How did we end up in this pact anyway?#k", 2);
    } else if (status == 2) {
	...
	qm.sendNextPrev("#b(Wait! That sounds just like that one dream you had... Did the two of you meet in a dream? Is it possible that the giant Dragon you saw in that dream was...Mir?#k", 2);
    } else if (status == 4) {
	...
	qm.sendNextPrev("#bI paid a price?#k", 2);
    } else if (status == 6) {
	...
	qm.sendNextPrev("#One in...spirit?", 2);
    } else if (status == 8) {
	...
	qm.sendNextPrev("#bI have no idea what you're talking about, but it sounds like a pretty big deal.#k", 2);
    } else if (status == 10) {
	...
	qm.sendNextPrev("#bBut it's peaceful here. There are no dangerous monster around.#k", 2);
    } else if (status == 12) {
	...
	qm.sendNextPrev("#bIt's not part of my five year plan. I'm just kidding, but seriously, I'm a farmer's son...#k", 2);
    } else if (status == 14) {
	...
	qm.forceStartQuest();
	qm.sendNext("Hehehe, alright then, master. Let's get to it!", 1);
    } else if (status == 16) {
	qm.sendNextPrev("#b(You're a bit confused, but you are now traveling with Mir the Dragon. Perhaps you'll go on an adventure together, like he said.)#k", 3);
    } else if (status == 17) {
