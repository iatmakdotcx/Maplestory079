...
function start(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        qm.sendNextPrev("嗯，我能帮助那么多的人，得到他们的喜爱，真开心。", 2)
    } else if (status == 2) {
        qm.sendNextPrev("(呃……这孩子真善良……)嗯，你的想法很积极向上。我有个建议给你，要不要听听？");
    } else if (status == 3) {
        qm.sendNextPrev("你突然那么一本正经的，让人好紧张。是什么建议？", 2)
    } else if (status == 4) {
        qm.sendNextPrev("我一直都很正经的。");
    } else if (status == 5) {
        qm.sendNextPrev("越看越吓人额？哈哈哈。", 2)
    } else if (status == 6) {
        qm.sendNextPrev("真被你打败了。行了，说说正题吧。咱俩被强制结成契约有一段时间了吧，现在应该可以缔结额外契约了。");
    } else if (status == 7) {
        qm.sendNextPrev("额外契约？", 2)
    } else if (status == 8) {
        qm.sendNextPrev("第一次契约不是我们自己决定的，而是受到命运的指引。但额外的契约必须要你我都同意才能缔结。");
    } else if (status == 9) {
        qm.sendNextPrev("呃……是粉红色又要加深了吗？不过要是能获得帮助更多人的力量，粉红色我忍了。", 2)
    } else if (status == 10) {
        qm.sendAcceptDeclineNoESC("那么就建立额外契约咯！你必须跟我同时缔结契约，集中精神吧？");
    } else if (status == 11) {
        if (qm.getJob() == 6511) {
            qm.changeJob(6512);
        }
