...
function start(mode, type, selection) {
	if (mode == 1)
	    ...
	    status--;
	if (status == 0) {
	    qm.sendNext("Over here! Hey, isn't this AWESOME? Oh, right, I haven't told you what it is. Okay, welcome to the #bHero's Gauntlet#k!\r\n...Wait. Wait wait wait. You're...NOT here for the Hero's Gauntlet?You want the #bMaple Castle clue#k? Oh...yeah...that...ha\r\nha...",1);
	} else if (status == 1) {
...
if (mode == -1) {
	...
	if (mode == 1)
	    ...
	    status--;
	if (status == 0) {
	    qm.sendNext("I knew you'd have fun! You can try it again anytime. You and me are buddies now, so I'll let you skip the line, hee hee! Oh, and you needed the \r#i3994657##b#t3994657##k right?\nWell, I'm done with it! Here, check this out...",1);
	} else if (status == 1) {
	    qm.sendNextPrev("A-hem.\r\n&quot;#bThe Black Mage continues to grow stronger. The school is in chaos. Maple Castle may fall sooner than we thought. We cannot lose a thousand years worth of magic knowledge to these villains.&quot;", 1);
	} else if (status == 2) {
	    qm.sendNextPrev("Pretty big downer, isn't it? The rest of the pages got shredded. #bIt's true that there was a secret magic power hidden away in Maple Castle#k. It must've been amazing, since the big, bad #bBlack Mage#k had his eye on it. ",1);	
	} else if (status == 3) {
