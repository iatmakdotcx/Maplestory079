/*
 ...
 脚本：武陵塔
 */
importPackage(Packages.handling.world.MapleParty);
  ...
  if (mapid == 925020001) {
    ...
    if (status == 0) {
      ...
      if (sel == 3) {
        ...
        if (cm.判断每日值("武陵塔") >= 每日可挑战 && !cm.getPlayer().isGM()) {
          cm.sendOk("今日无法继续挑战。");
          cm.对话结束();
          return;
        }
...
function 判断团队每日(name) {
  ...
  while (it.hasNext()) {
    var cPlayer = it.next();
    var mount_i = cPlayer.getBossLogD(name);
    if (mount_i > mount) {
      mount = mount_i;
    }
  }
